cat <<'EOF' > ~/ops-monitoring-stack/infrastructure/aws/user_data.sh
#!/bin/bash
...
# 2. INSTALACIÓN DE DOCKER & TAILSCALE
...
tailscale up --authkey=$TAILSCALE_AUTH_KEY --hostname=aws-sql-target --ssh --accept-routes

# 3. CONFIGURACIÓN DE PROMTAIL
mkdir -p /opt/lab/promtail
cd /opt/lab

echo ">>> [CONFIG] Generando configuración de Promtail..."
...
# 4. GENERACIÓN DEL DOCKER COMPOSE
